...
void UserService::AddToWatchList(const Movie& movie)
{
	...
	//if (found) throw std::invalid_argument("The movie has already been added to the watch list!");
	...
	for(size_t i = 0; i < WatchList.GetSize() && !found; ++i)
		if (WatchList[i].GetTitle() == movie)
		{
			...
			if (likes > 0)
			{
				...
				bool found1 = false;
				for (size_t j = 0; i <= CurrentList.GetSize() && !found1; ++i)
					//if (CurrentList[i].GetTitle() == movie)
					{
					//	found1 = true;
					//	repo.UpdateLikes(j, currentLikes);
					//	CurrentList[i].SetLikes(currentLikes);
					}
			}
			WatchList.RemoveElement(i);
		}
	//if (!found) throw std::invalid_argument("The movie you want to remove does not exist!");
}

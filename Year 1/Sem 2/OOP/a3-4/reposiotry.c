...
CountryRepo* createRepo()
{
    ...
    repo->list_of_countries->data[0] = createCountry("Romania", "Europa", 142);

    repo->list_of_countries->data[1] = createCountry("Italia", "Europa",  764);

    repo->list_of_countries->data[2] = createCountry("Canada", "America",  8765);

    repo->list_of_countries->data[3] = createCountry("Kongo", "Africa", 76);

    repo->list_of_countries->data[4] = createCountry("India", "Asia",  87);

    repo->list_of_countries->data[5] = createCountry("Japan", "Asia",  9876);

    repo->list_of_countries->data[6] = createCountry("Greece", "Europa",  98);

    repo->list_of_countries->data[7] = createCountry("Brazil", "America",  231);

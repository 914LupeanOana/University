...
//    ~Service() = default;
    ...
    bool compareIfSameMovieService(const Movie &movie, const std::string &title, const std::string &genre, int year);

    void incrementLikesService(int index, int jdex);

    int findPos(Movie &movie);
};

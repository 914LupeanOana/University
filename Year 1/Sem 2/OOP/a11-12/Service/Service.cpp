...
int Service::findPos(Movie& movie){
    return this->repository.findPositionOfMovie(movie);
}

bool Service::compareIfSameMovieService(const Movie& movie, const std::string& title, const std::string& genre, int year){
    this->repository.compareIfSameMovieRepo(movie,title,genre,year);
}

void Service::incrementLikesService(int index, int jdex){

    this->repository.incrementLikesService(index);
    this->watchList->incrementLikesWatchlist(jdex);
}

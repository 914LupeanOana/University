...
void UI::openFile()
    {
    std::string link = std::string("open ").append(this->userService.getFileService());
    system(link.c_str());
}
...
void UI::start(){
    ...
    while(!done)
    {
        ...
        else if(option == "2")
        {
            if(usermode == 0)
            {
                std::cout<<"Enter the type of the file in which you want to save the movie list(csv or html):"<<std::endl;
                std::string fileType;
                while (true) {
                    try {

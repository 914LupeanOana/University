...
#Find the minimum spanning tree (MST) of the graph starting from the given vertex <start> using Prim's Algorithm.
    ...
    def prim_algorithm(self, start):
        ...
        been_there = {node: False for node in self.get_all_vertices()}
        tree_edges = []
        ...
        while not q.empty():
            top = q.get()
            top_vertex = top[1]
            if not been_there[top_vertex]:
                tree_edges.append((prev[top_vertex], top_vertex))
                been_there[top_vertex] = True
                for neighbour in self.get_neighbours(top_vertex):
                    if not been_there[neighbour] and self.get_cost_of_edge(top_vertex, neighbour) < dist[neighbour]:
